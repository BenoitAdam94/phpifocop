...
if (
	...
	if (empty($msg)) {
		...
		if (!empty($id_article)) {
			// si $id_article b'est pas vide c'est un UPDATE
			$enregistrement = $pdo->prepare("UPDATE article SET reference = :reference, titre = :titre = :titre, categorie = :categorie, couleur = :couleur, taille = :taille, sexe = :sexe, prix = :prix, stock = :stock, description = :description, photo = :photo WHERE id_article = :id_article");
			// on rajoute le bind param
	...
	<div class="col-12">
		...
		?>
			...
			<form method="post" action="" enctype="multipart/form-data">
				<div class="row">
					<div class="col-6">
						<!-- recuperation de l'id article pour la modification -->
					...
					<div class="col-6">
						...
						<?php
						//recuperation de la photo de l'article en cas de modification pour la conserver si l'utilisateur n'en charge pas une nouvelle
						if (!empty($photo_actuelle)) {
							echo '<div class="form-group">';
							echo '<img src="' . URL . 'img/' . $photo_actuelle . '" class="w-25 img-thumbnail" alt="image de l\'article">';
							echo '<input type="text" name="photo_actuelle" value="' . $photo_actuelle . '">';
							echo '</div>';
						}
						?>
						<div class="form-group">
