<?php
$host_db = 'mysql:host=localhost;dbname=connexion'; 
$login = 'root'; 
$password = ''; 
$options = array(
...
if(isset($_POST['pseudo']) && isset($_POST['mdp'])) {
	...
	echo '<b>Mot de passe : </b>' . $mdp . '<hr>';
	
	// on place la requete SQL dans une variable pour pouvoir l'afficher
	
	//$req = "SELECT * FROM utilisateur WHERE pseudo = '$pseudo' AND mdp = '$mdp'";
	//echo '<b>Requete : </b>' . $req . '<hr>';
	
	// on execute la requete avec la methode query
	...
	if($resultat->rowCount() > 0) {
		...
		foreach($infos AS $indice => $valeur) {
			echo '<b>' . $indice . '</b> : ' . $valeur . '<br>';
		}
		
	} else {
		// s'il y a 0 ligne, le pseudo et / ou le mdp sont incorrects
		echo '<div style="background-color: red; color: white; padding: 20px;">Erreur sur les identifiants de connexion<br>Veuillez recommencer</div>';
	}
}
